...
if(decodifica($_POST["acao"]) == "adicionar"){
    ...
    /*
    for($i=0;$i<count($_POST['linha_produto']);$i++){

        // Insere o produto na categoria
        $linha = $pdo->prepare("INSERT INTO produto_linha (codigo_produto, codigo_linha) VALUES (:codigo_produto, :codigo_linha)");
        $linha->execute(array(
        ':codigo_produto' => $codigo_produto,
        ':codigo_linha' => $_POST['linha_produto'][$i]
        ));

        if(!$linha){
            $error_log[] = $linha->errorInfo();
        }
