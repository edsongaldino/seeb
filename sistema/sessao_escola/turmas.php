...
<?php
    ...
    JOIN escola ON escola.codigo_escola = turma.codigo_escola WHERE escola.codigo_escola = '".$codigo_escola."'";
    $result = $pdo->query( $sql_consulta_turma );
    $turmas = $result->fetchAll( PDO::FETCH_ASSOC );

    //consulta escolas
    $sql_consulta_escola = "SELECT escola.codigo_escola, escola.titulo_escola, escola.status, escola.arquivo FROM escola WHERE escola.codigo_escola = '".$codigo_escola."'";
    ...
    <script type="text/javascript">
        ...
        function removerTurma(id) {
           swal({
                    ...
                    window.location.href = "/sistema/sessao_escola/excluir.php?codigo_turma="+id;
                }, function(dismiss) {
                  // dismiss can be 'overlay', 'cancel', 'close', 'esc', 'timer'
                  if (dismiss === 'cancel') {
                    swal({
        ...
        function adicionarTurma(id) {
           swal({
                ...
                html: '<form id="myForm" method="post" action="/sistema/sessao_escola/adicionar_turma.php" ><div class="form-group">' +
                        ...
                        '<input id="input-field" name="escola" type="hidden" class="form-control" value="<?php echo $codigo_escola;?>" />' +
                    '</div></form>',
                showCancelButton: true,
                confirmButtonClass: 'btn btn-success',
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-12">
                                ...
                                <div class="card">
                                    ...
                                    <h4 class="card-title"><?php echo $escola["nome_escola"];?></h4>
                                        ...
                                        <div class="table-responsive">
                                            <table class="table table-striped">
                                                ...
                                                <tbody>
                                                    ...
                                                    <tr>
                                                        <td class="text-center"><?php echo $turma["codigo_turma"];?></td>
                                                        <td><?php echo $turma["nome_turma"];?></td>
                                                        <td><?php echo $turma["nivel_turma"];?></td>
                                                        <td class="td-actions text-right">
                                                            <a href="sessao_escola/lista.php?codigo_turma=<?php echo codifica($turma["codigo_turma"]);?>" title="Lista da Turma">
                                                                <button type="button" rel="tooltip" class="btn btn-warning" title="Lista da Turma">
                                                                    <i class="material-icons">list</i>
                                                                </button>
                                                            </a>
                                                            <a href="sessao_escola/editar_turma.php?codigo_turma=<?php echo codifica($turma["codigo_turma"]);?>">
                                                                <button type="button" rel="tooltip" class="btn btn-info" title="Editar Turma">
                                                                    <i class="material-icons">create</i>
                                                                </button>
                                                            </a>
                                                            <button type="button" rel="tooltip" class="btn btn-danger" title="Remover Turma" onclick="removerTurma('<?php echo codifica($turma["codigo_turma"]);?>')">
